        ...
        def _check_unit(string, default, index):
            ...
            if default.__contains__(line[0]):
                if line[1] not in default.get(line[0]):
                    print('Row ' + str(index + 1) + ': Expected Unit - (' + line[1]
                        + ') [For Item: ' + line[0] + ']')
                    file.loc[index, col] = '[New Unit]' + string
                    return 1
            elif line[0] != "-0":
                print(col + ' Row ' + str(index + 1) + ': Unknown Item: ' + line[0])
                file.loc[index, col] =  '[New Item]' + string
                return 1
            return 0
        ...
        for field in default:
            for row in range(len(file[field])):
                ...
                if cell not in default.get(field) and cell != "-0":
                    ...
                    file.loc[row, field] = '[New Item]' + str(cell)
                    bad = True
        if not bad:
...
def main():
    if argv[1] == "setup":
        ...
        writer = pd.ExcelWriter('highlight.xlsx', engine='xlsxwriter')
        ...
        # save
        writer.save()
        print('done. exported highlight.xlsx')
